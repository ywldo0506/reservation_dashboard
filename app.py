...
st.dataframe(
    result[['예약매장','총건수','체결','불가','체결률(%)','체결 성인','체결 초등','체결 미취학','체결 총인원','체결금액','손실금액']],
    use_container_width=True,
    hide_index=True,
    column_config={
        "체결금액": st.column_config.NumberColumn("체결금액", format="%d"),
        "손실금액": st.column_config.NumberColumn("손실금액", format="%d"),
        "체결률(%)": st.column_config.TextColumn("체결률(%)"),
    }
)
st.markdown('</div>', unsafe_allow_html=True)
